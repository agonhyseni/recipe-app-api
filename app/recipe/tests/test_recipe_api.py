...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_full_update(self):
        ...
        payload = {
            ...
            'description': 'New recipe despcription',
            'time_minutes': 10,
            'price': Decimal('2.50'),
        }
        url = detail_url(recipe.id)
    ...
    def test_recipe_other_users_recipe_errors(self):
        """Test trying to delete other user's recipe results in an error."""
        new_user = create_user(
    ...
    def test_clear_recipe_tags(self):
        """Testing clearing a recipes tags."""
    ...
    def test_recipe_with_new_ingredients(self):
        ...
        payload = {
            ...
            'time_minutes': 30,
            'price': Decimal('2.50'),
            'ingredients': [{'name': 'Floor'}, {'name': 'Water'}]
        }
        res = self.client.post(RECIPES_URL, payload, format='json')
    ...
    def test_recipe_with_existing_ingredients(self):
        ...
        ingredient_floor = Ingredient.objects.create(
            user=self.user, name='Floor')
        payload = {
            'title': 'Soup',
            'time_minutes': 60,
            'price': Decimal('4.50'),
        ...
        recipe = recipes[0]
        self.assertEqual(recipe.ingredients.count(), 2)
        self.assertIn(ingredient_floor, recipe.ingredients.all())
        for ingredient in payload['ingredients']:
            exists = recipe.ingredients.filter(
                name=ingredient['name'],
                user=self.user,
    ...
    def test_create_ingredient_on_update(self):
        ...
        payload = {'ingredients': [{'name': 'floor'}]}
        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format='json')

        self.assertEqual(res.status_code, status.HTTP_200_OK)
        new_ingredient = Ingredient.objects.get(user=self.user, name='floor')
    ...
    def test_filter_by_ingredients(self):
        ...
        ig1 = Ingredient.objects.create(user=self.user, name='Feta cheese')
        ig2 = Ingredient.objects.create(user=self.user, name='Chicken')
        r1.ingredients.add(ig1)
        r2.ingredients.add(ig2)
        r3 = create_recipe(user=self.user, title='Steak and mushrooms')

        params = {'ingredients': f'{ig1.id},{ig2.id}'}
        res = self.client.get(RECIPES_URL, params)
